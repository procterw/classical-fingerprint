import { Composer } from "./getMusicData";
import { composerBios } from './composer_bios.ts';

// Updated with all open opus recommended composers, except that with non essential works
export const selectedComposers: Array<Composer> = [
  {
    "id": "149",
    "name": "Adams",
...
.map((c) => {
  const bio = composerBios[c.id];
  if (!bio) {
    return { ...c, bio_preview: "", biography: "", wiki_url: "" };
  }
  return {
    ...c,
    bio_preview: composerBios[c.id].bio_preview,
    biography: composerBios[c.id].biography,
    wiki_url: composerBios[c.id].wiki_url,
  };
});
